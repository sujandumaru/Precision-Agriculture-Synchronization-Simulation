#!/usr/bin/env python3
"""Compare all seven policies on the full design, with paired tests.

Runs are paired: every (cell, replication) carries all seven policies evaluated
...
ORDER = ["last_write_wins", "cloud_preferred", "display_preferred",
         "version_vector_causal", "crdt_field_merge",
...
def main():
    ...
    print(f"{'policy':24} {'hi-risk silent':>14} {'total silent':>13} {'reviews':>9}")
    print("-" * 64)
    for p in ORDER:
        if p in P:
            print(f"{p:24} {mean(P[p]['high_risk_silent_overwrites']):14.4f} "
    ...
    for p in zero:
        ...
        print(f"  {p:24} {v:8.4f} reviews/run   reduction {1-v/mra:6.1%}")
